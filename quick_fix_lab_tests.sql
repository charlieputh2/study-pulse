-- Quick fix for missing get_lab_tests function
-- This will create the function if it doesn't exist
...
BEGIN
    IF NOT EXISTS (
        ...
        AND routine_name = 'get_lab_tests'
    ) THEN
        -- Create the function
        ...
        RAISE NOTICE 'get_lab_tests function created successfully';
    ELSE
        RAISE NOTICE 'get_lab_tests function already exists';
    END IF;
END $$;

-- Also create get_lab_test_detail function if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.routines 
        WHERE routine_schema = 'public' 
        AND routine_name = 'get_lab_test_detail'
    ) THEN
        ...
        -- Grant execute permission to public
        GRANT EXECUTE ON FUNCTION get_lab_test_detail TO public;
        GRANT EXECUTE ON FUNCTION get_lab_test_detail TO anon;
        
        RAISE NOTICE 'get_lab_test_detail function created successfully';
    ELSE
        RAISE NOTICE 'get_lab_test_detail function already exists';
    END IF;
END $$;
...
SELECT 'Functions are ready!' as final_status;
