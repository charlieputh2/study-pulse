-- Insert sample lab test data
INSERT INTO lab_tests (name, description, test_date, status, purity_result, test_method, product_id, report_url, certificate_number, lab_name, technician_name, notes) VALUES
('Tirzepatide Purity Analysis', 'Comprehensive HPLC analysis confirming pharmaceutical-grade purity and identity verification', '2024-01-15', 'passed', '99.8%', 'HPLC-UV', (SELECT id FROM products WHERE name LIKE '%Tirzepatide%' LIMIT 1), 'https://studypulse.com/reports/tirzepatide-purity-2024-01-15.pdf', 'SP-LAB-2024-001', 'Study Pulse Certified Lab', 'Dr. Sarah Chen', 'Batch #TIR2401 meets all quality specifications'),
('Semaglutide Potency Test', 'Bioassay testing for therapeutic potency verification and activity confirmation', '2024-01-12', 'passed', '99.5%', 'Cell-based Bioassay', (SELECT id FROM products WHERE name LIKE '%Semaglutide%' LIMIT 1), 'https://studypulse.com/reports/semaglutide-potency-2024-01-12.pdf', 'SP-LAB-2024-002', 'Study Pulse Certified Lab', 'Dr. Michael Roberts', 'Potency within 98-102% specification range'),
('Molecular Structure Verification', 'Mass spectrometry confirmation of molecular structure and molecular weight verification', '2024-01-10', 'passed', '100%', 'LC-MS/MS', (SELECT id FROM products WHERE name LIKE '%Tirzepatide%' LIMIT 1), 'https://studypulse.com/reports/tirzepatide-ms-2024-01-10.pdf', 'SP-LAB-2024-003', 'Study Pulse Certified Lab', 'Dr. Emily Watson', 'Molecular weight confirmed: 4753.6 Da'),
('Heavy Metals Screening', 'ICP-MS analysis for heavy metal contamination including lead, arsenic, cadmium, and mercury', '2024-01-08', 'passed', 'Safe', 'ICP-MS', (SELECT id FROM products WHERE name LIKE '%Semaglutide%' LIMIT 1), 'https://studypulse.com/reports/heavy-metals-2024-01-08.pdf', 'SP-LAB-2024-004', 'Study Pulse Certified Lab', 'Dr. James Lee', 'All heavy metals below USP limits'),
('Microbial Contamination Test', 'Sterility testing for bacterial and fungal contamination using USP <61> methods', '2024-01-05', 'passed', 'Negative', 'USP <61> Sterility Test', (SELECT id FROM products WHERE name LIKE '%Tirzepatide%' LIMIT 1), 'https://studypulse.com/reports/microbial-2024-01-05.pdf', 'SP-LAB-2024-005', 'Study Pulse Certified Lab', 'Dr. Lisa Anderson', 'No microbial growth detected after 14 days'),
('Endotoxin Analysis', 'LAL assay for endotoxin level verification using USP <85> methods', '2024-01-03', 'passed', '<0.1 EU/mg', 'LAL Assay', (SELECT id FROM products WHERE name LIKE '%Semaglutide%' LIMIT 1), 'https://studypulse.com/reports/endotoxin-2024-01-03.pdf', 'SP-LAB-2024-006', 'Study Pulse Certified Lab', 'Dr. Robert Taylor', 'Endotoxin levels well within specification'),
('Residual Solvent Analysis', 'GC-MS analysis for residual solvents according to ICH Q3C guidelines', '2024-01-01', 'passed', 'Compliant', 'GC-MS', (SELECT id FROM products WHERE name LIKE '%Tirzepatide%' LIMIT 1), 'https://studypulse.com/reports/solvents-2024-01-01.pdf', 'SP-LAB-2024-007', 'Study Pulse Certified Lab', 'Dr. Maria Garcia', 'All residual solvents below ICH limits'),
('pH and Stability Testing', 'pH measurement and accelerated stability testing under various conditions', '2023-12-28', 'passed', '7.4 ± 0.2', 'pH Meter & Stability Chamber', (SELECT id FROM products WHERE name LIKE '%Semaglutide%' LIMIT 1), 'https://studypulse.com/reports/ph-stability-2023-12-28.pdf', 'SP-LAB-2024-008', 'Study Pulse Certified Lab', 'Dr. David Kim', 'Stable for 24 months at 25°C/60% RH'),
('Particle Size Analysis', 'Laser diffraction analysis for particle size distribution and morphology', '2023-12-25', 'passed', 'D50: 45.2 μm', 'Laser Diffraction', (SELECT id FROM products WHERE name LIKE '%Tirzepatide%' LIMIT 1), 'https://studypulse.com/reports/particle-size-2023-12-25.pdf', 'SP-LAB-2024-009', 'Study Pulse Certified Lab', 'Dr. Jennifer White', 'Particle size within specification range'),
('Water Content Analysis', 'Karl Fischer titration for water content determination', '2023-12-22', 'passed', '2.3%', 'Karl Fischer', (SELECT id FROM products WHERE name LIKE '%Semaglutide%' LIMIT 1), 'https://studypulse.com/reports/water-content-2023-12-22.pdf', 'SP-LAB-2024-010', 'Study Pulse Certified Lab', 'Dr. Thomas Brown', 'Water content within specification');

-- Insert detailed test results for each lab test
...
-- Insert additional parameters for heavy metals test
INSERT INTO lab_test_results (test_id, parameter_name, parameter_value, unit, reference_range, status)
SELECT 
    lt.id,
    param,
...
-- Insert lab test images (sample URLs)
